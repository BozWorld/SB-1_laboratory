...
func _pas():
	...
	if randi_range(0,2) != 0 :
		...
		position.x += choix_x * puissance_deplacement
		position.y += choix_y * puissance_deplacement
		position.z += choix_z * puissance_deplacement
	else :
		
		position += (%Viseur.global_position - position).normalized() * puissance_deplacement
